...
# =====================================================================================
BACKTEST_CONFIG = {
    "data_file_path": str(DATA_DIR / 'ETHUSDT_1m_test_2020-01-01_to_2020-01-03.h5'),
...
def analyze_and_plot_performance(
    ...
    if config["plot_equity_curve"]:
        ...
        # 生成唯一文件名
        base_name = Path(config["equity_curve_path"]).with_suffix('').name
        output_path = EQUITY_OUTPUT_DIR / f"{base_name}_{timestamp}{param_str}.png"
...
def load_preprocessed_data(cache_key: str) -> Optional[tuple]:
    """加载预处理的数据缓存"""
    cache_file = CACHE_DIR / f"preprocessed_data_{cache_key}.pkl"
